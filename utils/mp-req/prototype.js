...
function requestP(options = {}) {
  ...
  const header = Object.assign({
    [req.sessionHeaderKey]: wx.getStorageSync(req.sessionHeaderKey),
  }, options.header);
      ...
      success(r1) {
        if (r1.code) {
          ...
          req.code2sessionId(r1.code)
            .then((r2) => {
              console.log(r2)
              const newSessionId = r2.data.result[0].sessionId;
              const newSessionHeaderId = r2.header["Set-Cookie"];
              console.log('get cookie success:',newSessionHeaderId);
              sessionId = newSessionId; // 更新sessionId
              wx.setStorageSync(req.sessionHeaderKey, newSessionHeaderId)
              // 保存sessionId
              wx.setStorage({
 ...
 */
function use(plugin) {
  return plugin.install(req, request, uploadFile);
}
